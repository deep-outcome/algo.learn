...
fn part(slc: &mut [usize], min_ix: usize, max_ix: usize) -> PivotalExRange {
    ...
    loop {
        ...
        slc.swap(gre_seek_ix, les_seek_ix);

        // organize all left pivot equals to
        // left start
        if slc[gre_seek_ix] == pivot {
            slc.swap(left_piv_ex_ix, gre_seek_ix);
            left_piv_ex_ix += 1;
        }

        // same for right side
        if slc[les_seek_ix] == pivot {
            slc.swap(les_seek_ix, right_piv_ex_ix);
            right_piv_ex_ix -= 1;
        }
    ...
    while read_ix < left_piv_ex_ix {
        slc.swap(read_ix, left_mid_piv_ex_ix as usize);
    ...
    while read_ix > right_piv_ex_ix {
        slc.swap(right_mid_piv_ex_ix, read_ix);
